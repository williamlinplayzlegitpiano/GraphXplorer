...
class Graph {
    ...
    private:
        struct graphImp {
            unordered_map<string, unsigned int> nodeMap;    
            vector<string> nodeList;
            vector<vector<pair<unsigned int, double>>> adjList; 
            unsigned int edgeCount = 0;

            unsigned int getNodeID(const string &label) const {
                unordered_map<string, unsigned int>::const_iterator iter = nodeMap.find(label);
                if (iter == nodeMap.end()) return (unsigned int)-1;
                return iter->second;
            }
        };
        graphImp imp;
